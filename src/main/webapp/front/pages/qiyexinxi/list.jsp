        ...
        <fieldset class="search-container" style="text-align: center;margin: 30px 0;">
          <form class="layui-form">
            ...
            <div class="layui-inline" style="margin-bottom: 10px;">
              ...
              <div class="layui-input-inline">
                <input type="text" name="qiyeleixing" id="qiyeleixing" placeholder="企业类型" autocomplete="off" class="layui-input">
              </div>
            </div>   
            <div class="layui-inline" style="margin-left: 30px;margin-bottom: 10px;">
              ...
              <button v-if="isAuth('qiyexinxi','新增')" @click="jump('../qiyexinxi/add.jsp')" type="button" class="layui-btn btn-theme">
        ...
        <div class="pro-lists-con">
          <ul class="after">
            <li v-for="(item,index) in dataList" v-bind:key="index">
              <a target="_self" :href="'../qiyexinxi/detail.jsp?id='+item.id">
                <div class="posi-img">
                  <img :src="item.tupian?item.tupian.split(',')[0]:''" width="385px" height="220px" />
                </div>
                <div class="remark">
                  <h5>{{item.qiyemingcheng}}</h5>
  ...
  <script>
    ...
    layui.use(['layer', 'element', 'carousel', 'laypage', 'http', 'jquery'], function() {
      ...
      function pageList() {
        ...
        if (jquery('#qiyeleixing').val()) {
          param['qiyeleixing'] = jquery('#qiyeleixing').val() ? '%' + jquery('#qiyeleixing').val() + '%' : '';
        }  
        // 获取列表数据
        http.request('qiyexinxi/list', 'get', param, function(res) {
